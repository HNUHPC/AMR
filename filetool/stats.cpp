...
void printleveldata_main(LevelData<FArrayBox>* leveldata, FILE *fp, int col)
{
	...
	{
		//        cout << "dit(): "  << dit[0] << endl;
		// cout << "dataIndex" << dit.i().datInd() << endl;
		...
		//pout() << "datalevelfabs.size()" << datalevelfabs.size() << endl;
		// int a = [(1,1)];
		...
		for (bit.reset(); bit.ok(); ++bit)
		{
			...
			//cout<<bit()[0]<< " "<<bit()[1]<< " ";
			// for(int i=0;i<18;i++)
			//cout<<datalevelfabs.get(bit(),0)<< " ";
			//cout<<endl;
			//if (dit.i().datInd() == 0)
			//{
			//	pout() << "bit(): " << bit() << endl;
			//    cout << "dlfs: " << datalevelfabs(bit(),0) << endl;
			//}
			//for (int ivar = 0; ivar < datalevelfabs.nComp(); ivar++)
			//{
			//if (dit.i().datInd() == 0)
			//  pout() << "dlfs: " << datalevelfabs(bit(),0) << endl;
			//cout << fixed << setprecision(20) << "thick:" <<thick[cnt] << endl;
		...
		for (dit.reset(); dit.ok(); ++dit)
		{
			...
			delete[] arrar;
			//for (int i = 0; i < 27744; i++)
			//cout << "array = " << thick[i] << endl;
		}
				...
				int maskEnd)
		{
			...
			//  cout<<"num of levels: "<<numLevels<<endl;
			bool has_src = false;
			...
			for (int lev = 0; lev < numLevels; lev++)
			{
				...
				// cout <<i++<<endl;
				// cout<< (*topography[lev])[dit](bit())<<endl;
				// }
		...
		int main(int argc, char* argv[]) {
			...
			{ // Begin nested scope
				...
				char filename[50];
				sprintf(filename,"datapoint_level_overhead.info");
				int col=0; 
