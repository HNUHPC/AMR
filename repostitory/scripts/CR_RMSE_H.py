...
fp=open("../repostitory/CR_RMSE_MaxE_H.txt","a")
for i in list1:
    print "*******************"
    print i 
    print "*******************"
   ...
   # print zfpdecmp
    ...
    NRMSE_baseline_zfp= np.sqrt(((vals_baseline_0 - vals_baseline) ** 2).mean())
   ...
   # print zfpdecmp
    ...
    NRMSE_levelRe_zfp= np.sqrt(((vals_levelRe_0 - vals_levelRe) ** 2).mean())
    ...
    #NRMSE= np.sqrt(((vals_1 - vals) ** 2).mean())/(max(vals)-min(vals))
    NRMSE_baseline_sz= np.sqrt(((vals_baseline_1 - vals_baseline) ** 2).mean())
    ...
    #NRMSE= np.sqrt(((vals_1 - vals) ** 2).mean())/(max(vals)-min(vals))
    NRMSE_levelRe_sz= np.sqrt(((vals_levelRe_1 - vals_levelRe) ** 2).mean())
