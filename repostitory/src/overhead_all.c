...
int main(int argc, char **argv)
{
	...
	for(int col=0;col<1;col++){
		...
		char compressed_name[50];
				sprintf(compressed_name,"Baseline_%d.zfp",list[col]);  
				start_t=clock();
				for( i=0;i<Num_run;i++){
		...
		double* decompressed;
		sprintf(compressed_name,"Baseline_%d.sz",list[col]);  
		start_t=clock();
		for( i=0;i<Num_run;i++){
			for(int i1=0;i1<datasize;i1++)
			{
		...
		for( i=0;i<Num_run;i++){
			...
			int status;
			compressed = readByteData(compressed_name, &outSize, &status);
			decompressed = SZ_decompress(SZ_DOUBLE, compressed, outSize,r5, r4, r3, r2, datasize);
			for(int i1=0;i1<datasize;i1++)
			{
